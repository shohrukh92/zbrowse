...
var headlessPath = "chrome-canary";
...
function getResourceTree(instance) {
  ...
  instance.Page.getResourceTree().then(v => {
      ...
      if (current.hasOwnProperty("childFrames")) {
        for (var i = 0; i < current.childFrames.length; i++) {
          ...
          networkTree.add(
            childURL,
            parentURL,
            networkData[childURL],
            networkTree.traverseBF
          );
    ...
    console.log(Object.keys(networkTree._root));

    fs.writeFileSync("result.json", JSON.stringify(networkTree));
